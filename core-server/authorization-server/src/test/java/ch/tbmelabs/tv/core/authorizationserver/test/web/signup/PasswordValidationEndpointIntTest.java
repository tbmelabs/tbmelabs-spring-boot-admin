...
public class PasswordValidationEndpointIntTest
  ...
  public void passwordWithoutNumbersFailValidation() throws Exception {
    mockMvc
        .perform(post(PASSWORD_VALIDATION_ENDPOINT).contentType(MediaType.APPLICATION_JSON).content(
            new JSONObject().put(PASSWORD_PARAMETER_NAME, PASSWORD_MISSING_NUMBERS).toString()))
        .andDo(print()).andExpect(status().is(HttpStatus.BAD_REQUEST.value()));
  }
  ...
  public void passwordWithoutSpecialCharShouldFailValidation() throws Exception {
    mockMvc
        .perform(
            post(PASSWORD_VALIDATION_ENDPOINT).contentType(MediaType.APPLICATION_JSON)
                .content(new JSONObject()
